...
change_data2 <- change_data1 %>% 
    ...
    mutate(cparty = paste(country, party) %>% str_replace_all(" ", "_") %>% tolower(),
           ...
           pedersen_lag = lag(pedersen),
           election = as.numeric(as.factor(paste(country, year)))) %>% 
    ungroup() %>% 
...
to_exclude <- c("votes", "votes_gain_abs", "votes_gain_rel", "country", "year", "party", "cparty")
...
```{r}
m1 <- lme4::lmer(data = change_data3, votes_gain_rel ~ change +
                     log_party_age + 
                     r_avg_vote + 
                     cabinet_party_last + 
...
lme4::lmer(data = change_data3, votes_gain_rel ~ change +
                     log_party_age + 
                     r_avg_vote + 
                     cabinet_party_last + 
                     enep_lag + name_exp + elec_shock + pedersen_lag +
                     (1|cparty) + (1|year) + (1|country)) %>% summary()

m2 <- lme4::lmer(data = change_data3, votes_gain_abs ~ change +
                     log_party_age + 
                     r_avg_vote + 
                     cabinet_party_last + 
                     enep_lag + name_exp + elec_shock + pedersen_lag +
...
lme4::lmer(data = change_data3, votes_gain_abs ~ change +
                     log_party_age + 
                     r_avg_vote + 
                     cabinet_party_last + 
                     enep_lag + name_exp + elec_shock + pedersen_lag +
