...
load("../data/new_change_data.rda") # run prepare_data.R to generate this file
...
get_ebal_weights <- function(df, treatment, exclude, targets = c(1, 2, 3)) {
    vars_to_keep <- df %>% 
        select(-one_of(c(exclude, treatment))) %>% 
        names()
    X <- df %>% select(vars_to_keep)
    if (targets == 3) {
        X1 <- matrixmaker(as.matrix(X)) %>% 
            as.data.frame() %>% 
            select(-one_of(names(.)[duplicated(t(.)) %>% which()]))
    } else if (targets == 2) {
        X1 <- getsquares(X)
     } else {
        X1 <- X
    }
    c_ebal <- ebalance(Treatment = df[[treatment]], X = X1)
    df <- as.data.frame(df)
    df$wt <- 1
    df$wt[df[[treatment]]==0] <- c_ebal$w
    
    return(df$wt)
}

to_exclude <- c("vote", "vote_gain_abs", "vote_gain_rel", "country", "year", "election", "election_pcy", "party", "cparty")

change_data3 <- change_data2 %>% 
    mutate(wt = get_ebal_weights(., treatment = "change", exclude = to_exclude, targets = 3))

to_exclude <- setdiff(names(change_data2), c("prev_vote", "change", "cabinet_party_last"))

change_data4 <- change_data2 %>% 
    mutate(wt = get_ebal_weights(., treatment = "change", exclude = to_exclude, targets = 3))
```


```{r}
m1 <- lme4::lmer(data = change_data3, vote_gain_rel ~ change +
                     log_party_age + 
                     r_avg_vote + 
                     cabinet_party_last + 
...
# without ebal matching weights: effect is smaller, p < .1
lme4::lmer(data = change_data3, vote_gain_rel ~ change +
                     log_party_age + 
                     r_avg_vote + 
                     cabinet_party_last + 
                     enep_lag + name_exp + elec_shock + pedersen_lag +
                     (1|cparty) + (1|election) + (1|country)) %>% summary()

m2 <- lme4::lmer(data = change_data3, vote_gain_abs ~ change +
                     log_party_age + 
                     r_avg_vote + 
                     cabinet_party_last + 
                     enep_lag + name_exp + elec_shock + pedersen_lag +
...
# without ebal matching weights, effect is much smaller (and p > .05)
lme4::lmer(data = change_data3, vote_gain_abs ~ change +
                     log_party_age + 
                     r_avg_vote + 
                     cabinet_party_last + 
                     enep_lag + name_exp + elec_shock + pedersen_lag +
...
m3 <- lme4::lmer(data = change_data4,
                 vote ~ prev_vote + change +
                     cabinet_party_last +
                     (1|cparty) + (1|election) + (1|country),
                 weights = wt) %>% summary()
```
